...
from helper_functions import get_major_courses, get_majors, compute_recommendations
...
def get_major_classes(major):
    return jsonify(get_major_courses(major))
...
def get_google_maps_api():
    return jsonify({"key": "AIzaSyBnCh2ugCrcyKDKT3HHry5KjkjS4ps4PT0"})
...
def get_ge_courses():
    try:
        with app.app_context():
            ...
            try:
                courses = (
                    session.query(CourseModel).filter(CourseModel.ge.isnot(None)).all()
                )

                last_update = session.query(LastUpdateModel).first()
                formatted_last_update = None
                if last_update:
                    formatted_last_update = last_update.update_time.strftime(
                        "%Y-%m-%d %H:%M:%S %Z"
                    )
                ...
                for course in courses:
                    ...
                    anyge_course["id"] = (
                        course.id + 5000
                    )  # Add 5000 to ID for AnyGE entries
...
def parse_transcript():
    ...
    try:
        ...
        return {"error": f"Failed to read transcript: {str(e)}"}, 400

    courses = re.findall(r"[A-Z]{2,4} \d{2,3}[A-Z]*", text)
    major = request.args.get("major", "Computer Science B.S.")

    return jsonify({"parsed_transcript": text, "courses": courses, "success": True})
